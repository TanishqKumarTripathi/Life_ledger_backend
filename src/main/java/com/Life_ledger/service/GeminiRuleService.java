...
public class GeminiRuleService {
    ...
    private JsonNode callGemini(String prompt) throws Exception {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (resp.statusCode() == 503 && attempt < maxRetries) {
                System.out.println("Gemini API overloaded, retrying in " + retryDelay + "ms (attempt " + attempt + "/" + maxRetries + ")");
                Thread.sleep(retryDelay);
                retryDelay *= 2;
                continue;
            }
            
            throw new RuntimeException("Gemini non-2xx: " + resp.statusCode() + " body:" + resp.body());
        }
    ...
    public JsonNode recurringRule(List<TransactionRuleDTO> list) throws Exception {
        ...
        String prompt = """
                ...
                - If same (merchant + amount) appears 2 or more times -> recurring = true for those txns
